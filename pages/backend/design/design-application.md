...
> Voir [Modèle domaine — API de Gestion de Régime Alimentaire](design-domain.md) pour les détails de `DeletedAt` et la grace period RGPD.
...
- **Workflow API-first.** Avant de coder les DTOs et services d'un ticket, définir les champs exacts des endpoints dans [Design technique — Couche API](design-api.md). Les DTOs découlent des contrats d'endpoint, pas l'inverse.
